...
- 源码系列
  ...
  - JUC、JVM、MySQL 复习、沈剑的架构课（3-4-5）⭐️ 
  - nacos、open-feign、gate-way、seata  源码解读
  - RocketMQ  6-8 ⭐️
  - Dubbo 9-10 ⭐️
  - K8s-云原生 11-12
- 新知识
...
- 原子性、可见性、有序性   volatile  执行重排序
  - MESI协议，是cpu缓存一致性的协议，大多数CPU厂商都实现了
...
- CAS、ABA
  ...
  - 底层就是native方法，调用的exchange方法指令，通过判断是否多核添加lock指令保证expxchg原子性
  
  - ABA ： java提供的atomicStampedBefreence解决
  
  - 自旋次数过多：
  
    - 次数过多，将线程挂起
    - 可以拆分粒度，longAdder  需要看具体的场景
...
- 阻塞队列
  - array是通过一个lock 进行阻塞 唤醒
  - linked 是通过两个lock 进行阻塞唤醒的  性能更高
...
- 单体架构的问题
  - 某个子系统的访问量过大，导致其他子系统访问出现问题
  - 拓展性差，新资源的分配力度不精确。
  - 
- 序列化 解决了数据存储和传输的问题
- 协议  解决数据格式标准和交互规则的问题，协议通常依赖序列化方式 http api使用json  kafka使用Avro
- **序列化是“如何表示数据”，协议是“如何正确解析数据”** 🚀

```markdown
1. client透明的调用远端的RPC服务
   1. 远端调用 不是本地调用 跨进程 跨JVM
      解决这个问题
      1. 网络通信（BIO NIO Mina Netty Web服务）
         Netty
      2. 协议
         应用层协议 Http1 Http2 
         传输层 TCP (私有协议) 格式 需要传输什么样的内容。要结合需求
      3. 序列化 
         Protobuf Thrift JDK JSON..
         
    2. 通过代理的方式 解决透明化的调用
       代理设计模式 ---> 动态代理 (jdk (Proxy.newProxyInstance) , Cglib, Javasist)
2. 自动发现RPC集群中的节点 [服务发现]
   注册中心（服务的注册，服务的发现）
   Zookeeper JAVA Curator 
3. 负载均衡 
   客户端的负载均衡
   1. 拉数据 （注册中心zk） zkclient curator
   2. 监听数据的变化  zkclient curator
   3. 获取了数据后 ，通过某些负载均衡的算法 访问某一个服务节点
      a) roundrobin 
      b）random
      c) weight
4. 集群容错
   FailFast,抛出异常
   FailOver，重试 （延迟队列 定时任务 ---》（时间轮））
...
- zk 主节点 leader 从节点 flower
- 如何选举zk主节点 超过半数的节点为leader节点  选举算法
- 主节点 修改数据 从节点 读数据 
  - 数据通过的过程中 如果超过一半以上同步 就认为数据可读
- 容错模式  过半节点出现问题，zk集群服务中断
- 建议部署的是奇数 3个

Client和集群的通信

- 查询
  - 访问任意的zk节点 进行读取数据 ,  可能有数据同步不及时 获取不到
- 修改
  - client节点访问leader节点 leader完成造作，同步到其他节点。过半的就认为成功。
  - Client节点访问flower节点 flwooer节点找到ledaer完成后续操作。
...
- 服务注册/服务名/ip+port (多个)   前两级持久节点 最后是临时节点
- 每个服务都当作一个zk 的客户端，服务启动之后 zk的客户端代码在zk的树状结构中，创建临时节点，完成服务注册
- 添加一个心跳检测机制

服务发现

- 根据服务名从注册中心中获取服务列表
- 根据服务列表 选择负载均衡算法 进行选择一个实例进行访问
- 针对服务列表的监控   不是pull , 针对push的方式 zk主动推送 watch机制

ZK集群的选举过程

- 初始情况 每个节点都是looking 状态
- 选举过程中 有一个节点选举过半就是主节点 其他就是从节点  主节点：leader 从节点：follower  Observer观察者 (只同步数据 不投票)
- 1.id服务器id。 -data-myid文件   id小的服务器投递给id大的服务器节点
- 2.zxid  zk事务id 值越大优先级越高
- 3.Epoch 投票论数  值越大优先级越高
- Epoch > zxid > id 
- 集群初始化
- 集群运行中，主出现问题
...
- 主从集群中 经常发生   出现2个部分， 一部分是有主节点 另外一部分超过半数的节点都是从节点， 就出现脑裂问题。
- 通过epoch的轮数 选举最大的 就可以解决

容错

- Fail Fast 抛异常    FailOver 使用其他节点
...
- Service/ Chain Layer (LangChain) -> Model Layer 模型层 (LLM提供商) -> Storage Layer  存储层  (向量数据库)
...
- 当随机事件发生的次数足够多时，发生的频率才会趋近于预期的概率
- 每件事情都是独立的事件。
...
- 成事能力
  - 能者多劳 劳者多能， 去经历事情，只看书只是学习的过程。承担事情。
  - 更大的责任，充实的人生，附加更好的收益。
  - 精于算计无法晋升，勇于承担才是正途，不算而算。
  - 克服惰性，终生成长的思维
  - 面对挫折，建设强大内心
